...
def saflow_preproc(filepath, savepath, reportpath):
    ...
    closefig(fig)
    fmax = 0.40 ## correlation threshold for ICA components (maybe increase to 40. ?)
    ## FIND ECG COMPONENTS
    ecg_epochs = create_ecg_epochs(raw_data, ch_name='EEG059')
    ecg_inds, ecg_scores = ica.find_bads_ecg(ecg_epochs, ch_name='EEG059', method='ctps', threshold=fmax)
    ...
    eog_inds, eog_scores = ica.find_bads_eog(eog_epochs, ch_name='EEG057', method='ctps', threshold=fmax)
